...
describe('Header', () => {
    ...
    it('renders a link to the Settings page if displayBack is false', () => {
        ...
        expect(links).toHaveLength(2); // check that there is 1 link
        expect(links[1].href).toContain("/setting"); // check that the link is to the settings page
    });
    it('renders a "Back" link if displayBack is true', () => {
        render(<Header displayBack={true} />);
        const links: HTMLAnchorElement[] = screen.getAllByRole("link"); // create an array of links
        expect(links).toHaveLength(2); // check that there is 1 link
        expect(links[1].href).toContain("/"); // check that the link is to the home page
